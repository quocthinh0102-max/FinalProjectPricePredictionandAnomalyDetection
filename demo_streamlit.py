    ...
    **Các bước:**
    ...
    with tab_user:
        ...
        try:
            ...
            with col_u_num1:
                nam_dk_a = st.number_input("Năm đăng ký", min_value=1980, max_value=2025, value=2015, step=1)
            with col_u_num2:
                so_km_a = st.slider("Số Km đã đi", min_value=0, max_value=500000, value=50000, step=1000)
        except Exception:
            # Giữ nguyên source Code cho phần này theo yêu cầu của user
            st.error("❌ Data mẫu bị lỗi hoặc thiếu cột thông số xe.")
            st.stop()
